...
api_key_id = 'PKGF8HFBHWSITXJONEK6'
api_secret = 'uiPdTutW1tEKfYBj4Mb3cRLcqgJtU9RcXtavo3VK'
...
Algorithm initialized variables:
 ...
 HighVar %s''' %
    ...
    # Long close price average.
    LongAvg2 = SimpleMovingAverage(
        inputs=[USEquityPricing.close],
        window_length=75,
        mask=base_universe
    )
    
    percent_difference = ((ShortAvg+ ShortAvg2 ) - (LongAvg +LongAvg2 )) / (LongAvg +LongAvg2 )    
...
def log_open_order(StockToLog):
    ...
    for stock, orders in list(oo.items()):
